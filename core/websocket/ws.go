...
type ws interface {
	...
	Kick(key string, message []byte, mType ...int) error
	Close()
...
func (kws *Websocket) Kick(key string, message []byte, mType ...int) error {
	_ws := pool.get(key)
	if _ws != nil && _ws.IsAlive() {
		_ws.EmitTo(key, message, mType...)
		_ws.Close()
	}
	return nil
}
